...
> `POST /api/v1/notifications/send-push-via-bot/` (запуск рассылки через
> Telegram-бота) полностью удалён вместе со всей Telegram-логикой проекта —
> роут, `SendPushViaBotView` и `SendPushViaBotSerializer` больше не существуют
> в коде. Единственный способ запустить рассылку теперь — `POST /bulk-push/`
> выше или Django Admin (см. следующий раздел).
