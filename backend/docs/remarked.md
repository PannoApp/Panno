...
> ⚠️ **2026-07-14: `GetToken` временно не используется.** Проверено эмпирически:
> `point` у `GetToken` ломает метод (`{"status":"error","message":"Unknown
> error"}`) при **любом** переданном значении — нашей точке 303450, примере из
> их же спеки (7999999), случайном числе. Без `point` метод не ошибается, но
> возвращает токен с неверным скоупом — `GetSlots` этим токеном отдаёт залы/
> столы (304/305, 43 стола), не совпадающие с реальной схемой зала в кабинете
> Remarked (357400452/357400453, 31 стол). Подтверждено сравнением с рабочим
> примером от поддержки Remarked (токен `7de3dc43d42a6fbbf9f12e911ea2b48b`,
> тем же методом `GetSlots` вернул правильные 357400453/реальные table id).
>
> Пока используем этот токен как статический (`REMARKED_RESERVES_STATIC_TOKEN`
> в `.env`) — `get_token()` возвращает его напрямую, `GetToken` не вызывается
> вообще. Это сознательный временный компромисс на время тестирования (см.
> обсуждение с Remarked) — токен может быть отозван без предупреждения, тогда
> все бронирования разом начнут падать с ошибкой Remarked. Вернуть на
> динамический `GetToken` с кешированием и retry-once на 401, когда Remarked
> починит обработку `point`.
...
Оборачивает `RemarkedReservesClient` и добавляет типизированные методы для
брони поверх статического токена (см. предупреждение выше).
...
Доступные методы: `get_token(force_refresh=False)` (сейчас всегда возвращает
`settings.REMARKED_RESERVES_STATIC_TOKEN`, `force_refresh` ни на что не
влияет — оставлен для совместимости интерфейса), `get_slots(...)`,
...
Ошибка от Remarked (в т.ч. 401) просто пробрасывается — retry-once с
обновлением токена убран вместе с `GetToken` (обновлять нечего, токен
статический). Повтор всей задачи — на уровне `@shared_task`/`autoretry_for`
(как и у остальных интеграций), либо, для `sync_reserve_statuses`, ошибка по
одной брони просто пропускается до следующего планового запуска.
...
> оказалось неверным выводом — "нет ошибки" ≠ "это наши данные". Прямое
> сравнение с рабочим примером от поддержки Remarked показало: токен без
...
> 357400453, 31 стол). См. предупреждение в разделе «ReservesClient» выше —
> сейчас используется отдельный статический токен, полученный от поддержки
> напрямую, `GetToken` не вызывается вообще.
