...
> ✅ **Разгадано 2026-07-14: дело было в заголовке `Referer`, не в `point`.**
> `GetToken` с `point` ломался (`Unknown error`) при любом значении `point` —
> но не из-за самого `point`, а потому что сервер не может сверить точку с
> разрешённым IP без заголовка `Referer`. С `Referer`, выставленным в реальный
> исходящий IP сервера (`REMARKED_RESERVES_REFERER` в `.env`), `GetToken` с
> `point` возвращает токен, корректно заскоупленный на нашу точку — `GetSlots`/
> `CreateReserve`/`GetReserveByID` этим токеном отдают реальные данные,
> совпадающие со схемой зала в кабинете Remarked (357400452/357400453,
> 31 стол), а не легаси 304/305. Проверено живьём: `GetReserveByID` реальной
> брони вернул реальный статус (`confirmed`) и реальный стол («33», зал
> «1 Этаж»).
>
> **Важно:** `REMARKED_RESERVES_REFERER` должен совпадать с реальным исходящим
> IP сервера, который сейчас его гоняет — обновлять при смене сервера/деплоя
> (см. `curl -s https://api.ipify.org` для проверки текущего исходящего IP).
> Простого совпадения значения недостаточно — **Remarked дополнительно держит
> собственный белый список IP** для точки 303450 на своей стороне (сервер с
> `Referer`, которого нет в их списке, всё равно получит `Unknown error`).
> При смене сервера — помимо обновления `.env`, нужно попросить поддержку
> Remarked добавить новый IP в список для точки 303450. Стейджевый IP
> `185.98.7.12` уже добавлен и подтверждён рабочим (2026-07-15).
...
токен через `GetToken` (с `point` и `Referer`), автоматически повторяет
вызов один раз с обновлённым токеном при 401.
...
(`comment`, `tags`) или считаются самим Remarked (`bonuses`,
`amount_spent`), обновлениями профиля из приложения не затираются —
текущая реализация `create_or_update()` (шлёт только известные поля)
корректна и не нуждается в доработке «слать полное состояние».

> ⚠️ **Уточнение (2026-07-23):** `subscriptions`, в отличие от `comment`/
> `tags`, оказался полем, которое **обязательно нужно слать самим** — без
> него гость технически имеет `firebase_token`, но не подписан на push-канал
> Remarked, и их рассылки до него молча не доходят. Подробности и фикс — см.
> `docs/users.md#post-storecustomercreate--подтверждён-partial-update`.
...
(и с верным, и без Authorization-заголовка, и с любым другим значением
`point` — проверено на примере из их же спеки 7999999 и на случайных числах)
отвечает `{"status":"error","message":"Unknown error"}`, а
`{"method": "GetToken"}` без `point` — валидным токеном.

> ⚠️ **Исправление (2026-07-14):** здесь раньше утверждалось, что скоуп
> токена без `point` "уже соответствует нашей точке", потому что `GetSlots`/
> `GetReservesByPhone` этим токеном не отвечали ошибкой авторизации. Это
> оказалось неверным выводом — "нет ошибки" ≠ "это наши данные". Токен без
> `point` стабильно резолвится в несвязанный с реальной схемой зала набор
> данных (залы 304/305, 43 стола), а не в реальную схему (357400452/
> 357400453, 31 стол).
>
> ✅ **Настоящая причина ошибки `point` найдена в тот же день:** дело было не
> в `point`, а в отсутствующем заголовке `Referer`. С `Referer`, выставленным
> в реальный исходящий IP сервера, `{"method": "GetToken", "point": 303450}`
> отрабатывает штатно и возвращает токен с правильным скоупом — см.
> предупреждение в разделе «ReservesClient» выше, сейчас `get_token()`
> передаёт и `point`, и `Referer` (`REMARKED_RESERVES_REFERER`).
