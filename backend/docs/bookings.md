...
Повторный запуск безопасен — Beat всё равно запустит следующую итерацию через 15 минут, дублей не возникает, так как окно выборки смещается вместе со временем.
