...
### Как зал влияет на создание брони

Если у `TableBooking` заполнен `remarked_room_id` (гость выбрал зал в форме),
`create_reserve_in_remarked` (`apps/bookings/tasks.py`) перед вызовом
`CreateReserve` дополнительно вызывает `pick_table_for_room(...)`, чтобы
найти свободный стол именно в этом зале на точные дату/время/кол-во гостей, и
передаёт его как `reserve['table_ids'] = [table_id]`. Если подходящего стола
не нашлось (зал разобрали между проверкой доступности и созданием брони, или
сам вызов `pick_table_for_room` упал с ошибкой Remarked) — `table_ids` просто
не передаётся, бронь всё равно создаётся, но с автоподбором стола по всему
ресторану (без ошибки для гостя). Без выбранного зала (`remarked_room_id`
пуст) — `pick_table_for_room` вообще не вызывается, поведение прежнее.
