...
**`subscriptions` (добавлено 2026-07-23):** одного `firebase_token` в теле
недостаточно, чтобы гость реально получал push-рассылки Remarked через FCM —
без явной подписки их виджет рассылок отправляет "успешно", но гостю ничего
не приходит (подтверждено Remarked после того, как мы показали, что тот же
самый ключ и токен устройства реально доставляют push при прямом вызове
FCM API). Поэтому вместе с `firebase_token` теперь всегда отправляется
`subscriptions: {"type": "firebase", "sub_type": "all"}`.
